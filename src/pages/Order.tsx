...
import { Header, Alert as OrderConfirmationAlert, Popover } from 'components'
import { setActiveRequestsPresence } from 'session'
import {
  getDeliveryAddressForNextOrder,
...
class Component extends React.Component<Props> {
  ...
  onConfirmOrder = () => {
    ...
    showLoading()
    Requests.post(endPoints['item-requests'], payload)
      .then((response: any) => {
        if (response.error) {
  ...
  setOrderConfirmationVisibility = (v: boolean) =>
    this.setState({ orderConfirmationShown: v })
  ...
  render() {
    const { cost, orderConfirmationShown, quantityModifyItem, selectedItems } =
      this.state
    ...
    return (
      <IonPage className="order">
        ...
        >
          <ItemQuantity item={item} onSubmit={this.onQtyChangeSubmitted} />
        </Popover>
...
const ItemQuantity: React.FC<ItemQuantityProps> = ({ item, onSubmit }) => {
